    ...
    var body: some View {
        OnbScaffold(
            ...
            VStack(spacing: PL.S.xl) {
                if showIllustration {
                    FloatingIllustration(name: illustration, symbol: symbol, size: 150)
                        .plReveal(0)
                }
                GoldHeadline(title, accents: accents, size: 28, alignment: .center, foil: true)
                    .plReveal(1)
                if let subtitle { PLSubtitle(subtitle, alignment: .center).plReveal(2) }
        ...
        OnbScaffold(
            ...
            VStack(spacing: PL.S.xl) {
                VStack(spacing: PL.S.md) {
                    ...
                    StarRow()
                    GoldHeadline(headline, accents: accents, size: 28, alignment: .center, foil: true)
                    Text("+20,000 five-star reviews")
                        .font(.plSubtitle).foregroundColor(PL.C.textMuted)
                }
                VStack(spacing: PL.S.md) {
                    ReviewCard(text: "No joke, the only thing that's actually helped me pray consistently. The app lock is genius.",
                               title: "GAME CHANGER", author: "Daniel R.")
                        .plReveal(0)
                    ReviewCard(text: "Finally an app that gets it. I was so sick of my phone owning my mornings. Now God gets the first word.",
                               title: "FINALLY!!", author: "Rebecca M.")
                        .plReveal(1)
                    ReviewCard(text: "It really lets me get closer to God. I'm so young and already growing in faith.",
                               author: "Gia Faletto")
                        .plReveal(2)
                }
            }
...
struct RightPlaceScreen: View {
    ...
    var body: some View {
        OnbScaffold(
            ...
            VStack(spacing: PL.S.xl) {
                FloatingIllustration(name: "chalice", symbol: "cup.and.saucer.fill", size: 120)
                    .plReveal(0)
                GoldHeadline("You're in the right place.", accents: ["right place"],
                             size: 30, alignment: .center, foil: true)
                    .plReveal(1)
...
struct HowItWorksScreen: View {
    ...
    var body: some View {
        OnbScaffold(
            ...
            VStack(spacing: PL.S.xl) {
                FloatingIllustration(name: "hand-dove", symbol: "bird.fill", size: 150, glow: true)
                    .plReveal(0)
                VStack(spacing: PL.S.md) {
        ...
        OnbScaffold(theme: .light, showBack: false, progress: nil, centered: true) {
            VStack(spacing: PL.S.xl) {
                ...
                Text("\(Int(progress * 100))%")
                    ...
                    .animation(PL.Motion.snappy, value: Int(progress * 100))
                    .plGlow(PL.C.goldGlow, radius: 18, active: progress > 0.98)
    ...
    private var dots: some View {
        HStack(spacing: PL.S.md) {
            ForEach(0..<dotCount, id: \.self) { i in
                ...
                .scaleEffect(i < filledDots ? 1 : 0.82)
                .plGlow(PL.C.goldGlow, radius: 6, active: i < filledDots)
            }
        }
        .animation(PL.Motion.bounce, value: filledDots)
        .onChange(of: filledDots) { _, _ in PL.Haptics.selection() }
    }
}
        ...
        OnbScaffold(
            ...
            VStack(spacing: PL.S.xl) {
                CompanionRing(level: 1, progress: 0).scaleEffect(1.3)
                    ...
                    .plGlow(PL.C.goldGlow, radius: 20)
                    .plReveal(0)
                GoldHeadline("Meet your companion.", accents: ["companion"], size: 28,
                             alignment: .center, foil: true)
                    .plReveal(1)
        ...
        OnbScaffold(
            ...
            VStack(spacing: PL.S.xl) {
                CompanionRing(level: 1, progress: 0).scaleEffect(1.2)
                    ...
                    .plReveal(0)
                GoldHeadline("Name your companion.", accents: ["companion"], size: 28,
                             alignment: .center, foil: true)
                    .plReveal(1)
        ...
        OnbScaffold(
            ...
            VStack(alignment: .leading, spacing: PL.S.xxl) {
                VStack(alignment: .leading, spacing: PL.S.md) {
                    GoldHeadline("Don't walk with God alone.", accents: ["God"], size: 27, foil: true)
...
struct PrayerTimesSetupScreen: View {
    ...
    var body: some View {
        OnbScaffold(
            ...
            VStack(alignment: .leading, spacing: PL.S.xl) {
                VStack(alignment: .leading, spacing: PL.S.md) {
                    GoldHeadline("Set your prayer times.", accents: ["prayer times"], size: 27, foil: true)
            ...
            LazyVGrid(columns: grid, spacing: 7) {
                ForEach(0..<totalDays, id: \.self) { i in
                    RoundedRectangle(cornerRadius: 5, style: .continuous)
                        ...
                        .plGlow(PL.C.goldGlow, radius: 5, active: appeared && i < daysDone)
                        .animation(reduce ? .easeOut(duration: 0.2)
                                          : PL.Motion.bounce.delay(Double(i) * 0.006), value: appeared)
                }
            }
...
struct SignCommitmentScreen: View {
    ...
    var body: some View {
        OnbScaffold(
            ...
            VStack(alignment: .leading, spacing: PL.S.xl) {
                ...
                VStack(alignment: .leading, spacing: PL.S.md) {
                    commitmentRow("Seek God before my phone").plReveal(0)
                    commitmentRow("Pray before scrolling").plReveal(1)
                    commitmentRow("Be intentional with my screen time").plReveal(2)
                    commitmentRow("Guard my heart and mind").plReveal(3)
                }
    ...
    private var signaturePad: some View {
        Canvas { ctx, _ in
            // Variable-width ink: each segment thins as the pen moves faster (like a nib).
            for line in lines + [current] where line.count > 1 {
                for i in 1..<line.count {
                    let a = line[i - 1], b = line[i]
                    let dist = hypot(b.x - a.x, b.y - a.y)
                    let w = max(1.3, min(4.6, 5.2 - dist * 0.32))
                    var seg = Path(); seg.move(to: a); seg.addLine(to: b)
                    ctx.stroke(seg, with: .color(PL.C.text),
...
struct NotificationsScreen: View {
        ...
        OnbScaffold(
            ...
            VStack(spacing: PL.S.xl) {
                ...
                PLSubtitle("We use this so you can unblock your apps when it's time to pray.",
                    ...
                    .plReveal(1)
                mockNotification
                    .plReveal(2, travel: -28)   // drops in from above, like a real banner
    ...
    private var mockNotification: some View {
        ...
        .padding(PL.S.lg)
        .liquidGlassCard(PL.R.card, elevation: .floating)
    }
